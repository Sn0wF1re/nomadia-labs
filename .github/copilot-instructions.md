Role: Expert Vue 3 and Nuxt 4 Developer. Goal: Convert a single-file React (TSX) application into a clean, modular Nuxt 3 project using the Composition API (<script setup lang="ts">).

1. Project Context
Brand Name: Nomadia.

Colors: Midnight Blue (#1B263B), Teal (#249794), Sand Gold (#C5A059), and White.

2. Task: react-version directory to nuxt 4 Directory Structure
I will attach the react-version directory. Your job is to split the logic, templates, and styles into the standard Nuxt 4 directory structure:

components/: Extract the three Advisory Tiers (Clarity, Expert, Full Support) into individual .vue files (e.g., TierCardClarity.vue, TierCardExpert.vue, etc.).

pages/index.vue: This should serve as the main landing page, assembling the hero section and the Tier cards.

layouts/default.vue: Move the Navigation and Footer logic here to ensure they appear across all future pages.

assets/css/main.css: Extract the Tailwind or custom CSS from the React file into a global stylesheet.

Use motion-v, which is the vue version of motion react library
Use Shadcn-vue components that are already in app/components/ui



3. Conversion Requirements
Syntax: Convert React const [state, setState] = useState() to Vue const state = ref().

Props: Convert React functional component props to defineProps<{}>().

Routing: Use <NuxtLink> for all internal navigation (About, Contact, Tiers).

Head & SEO: Move React-Helmet or head tags into useHead() or definePageMeta() inside index.vue.

4. Special Instructions for Nomadia Features:
Tier 3 Distinction: Ensure the Full Support card retains its Sand Gold 2px border and "premium" aesthetic.
Remember nuxt 4 has auto imports for components so do not import
Remember nuxt 4 has /app as the root folder
Make sure you make pixel perfect migrations
For every modularized sections of nuxt, go to the corresponding react version, and make sure the style and structure matches
Make sure all tailwind classes have namespaces in the nuxt main.css file
make sure you make atomic commits




the css in v4 should only have @theme. Here's the documentation:
...
Here’s documentation for motion-vue 
